...
Evacuation Pause: Fully Young (이 단계는 Young Gen만 해당)
...
11. GC이벤트의 지속시간을 다른 분류로 측정한 것이다.
    1. user : 콜렉션 동안 가비지콜렉터가 소비한 총 CPU타입
    2. sys : OS콜로 보낸 시간과 시스템 이벤트를 기다리는 동안 보낸 시간
    3. real : 애플리케이션이 멈춰있는 동안의 clock time. GC동안 병렬화된 활동으로 이 숫자는 이론적으로 가비지콜렉터에 의해 사용된 쓰레드수로 (user + sys)를 나눈 것에 가깝다. 로그는 8개 쓰레드가 사용되었다. 어떤 활동은 병렬화되지 않았기 때문에 이 수치는 일정양에 대한 비율을 항상 초과한다.
